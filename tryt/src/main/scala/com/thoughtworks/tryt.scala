...
object tryt {
  ...
  private[tryt] trait OpacityTypes {
    ...
    def apply[F[_], A](fTryA: F[Try[A]]): TryT[F, A]

    def unwrap[F[_], A](tryT: TryT[F, A]): F[Try[A]]

    def toCovariantTryT[F[+ _], A](fTryA: F[Try[A]]): CovariantTryT[F, A]

    def fromCovariantTryT[F[+ _], A](tryT: CovariantTryT[F, A]): F[Try[A]]

    type CovariantTryT[F[+ _], +A]

    def covariantTypeClass[TypeClass[_[_]], F[+ _]](typeClass: TypeClass[TryT[F, ?]]): TypeClass[CovariantTryT[F, ?]]
  ...
  private[tryt] val opacityTypes: OpacityTypes = new OpacityTypes {
    ...
    type CovariantTryT[F[+ _], +A] = F[Try[A]]
    type TryT[F[_], A] = F[Try[A]]

    override final def apply[F[_], A](fTryA: F[Try[A]]): TryT[F, A] = fTryA

    override final def unwrap[F[_], A](tryT: TryT[F, A]): F[Try[A]] = tryT

    override final def toCovariantTryT[F[_], A](fTryA: F[Try[A]]): TryT[F, A] = fTryA

    override final def fromCovariantTryT[F[_], A](tryT: TryT[F, A]): F[Try[A]] = tryT

    override def covariantTypeClass[TypeClass[_[_]], F[+ _]](
        typeClass: TypeClass[TryT[F, ?]]): TypeClass[CovariantTryT[F, ?]] = typeClass
  }
  ...
  object covariant {
    object TryT {
      def apply[F[+ _], A](fTryA: F[Try[A]]) = opacityTypes.toCovariantTryT(fTryA)
      def unapply[F[+ _], A](tryT: TryT[F, A]) = Some(opacityTypes.fromCovariantTryT(tryT))
      implicit def covariantTryTTypeClass[TypeClass[_[_]], F[+ _]](
          implicit typeClass: TypeClass[invariant.TryT[F, ?]]): TypeClass[TryT[F, ?]] =
        opacityTypes.covariantTypeClass(typeClass)
    }

    /** The covariant version of [[TryT]].
      *
      *@example xxx
      * {{{
      ...
      * @template
      */
    type TryT[F[+ _], +A] = opacityTypes.CovariantTryT[F, A]
  }
}
